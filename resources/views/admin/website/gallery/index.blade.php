...
@section('content')
  ...
  <div class="content-wrapper bg-white pt-1" style="min-height: 1416.81px;">
    ...
    <div class="card card-cascade narrower">
      ...
      <!--Card image-->
        <h2 class=" card card-header white-text text-center mx-3 bg-blue">Gallery</h2>
      ...
      <div class="px-4">
        ...
        <div class="table-wrapper">
            <a href="{{route('admin.gallery.create')}}" class=" btn bg-green text-bold m-2" style="font-size: 18px;">+ Add New Image</a>
          <!--Table-->
            ...
            <tr deletedid="{{$gallery->id}}">
                <td><img src="{{asset('images/gallery/'.$gallery->image)}}" alt="{{$gallery->id}} Shortcuts" width="150" height="150" ></td>
                <td statusactive="{{$gallery->id}}" style="color: @if($gallery->status=='0') red @else green @endif">@if($gallery->status=='0')Not Active @else Active @endif</td>
              <td>
                  <div>
                  <a href="{{route('admin.gallery.edit',$gallery->id)}}" class="btn btn-outline-white btn-rounded btn-sm px-2" style="color:#007bff;">
                    <i class="fas fa-pencil-alt mt-0"></i>
                  </a>
                  <a href="{{route('admin.gallery.delete')}}" deleteid="{{$gallery->id}}" class="gallerydelete"  style="color:red;">
                    <i class="far fa-trash-alt mt-0"></i>
                  </a>
                </div>
              </td>
                <td>
                    <a href="{{route('admin.gallery.changestatus')}}" activateid="{{$gallery->id}}" class=" galleryactivate btn btn-@if($gallery->status=='0')primary @elseif($gallery->status=='1')danger @endif  btn-sm px-2 m-0" style="color:white;">
                        @if($gallery->status=='0')Activate
                        @elseif($gallery->status=='1')Deactivate @endif
                    </a>
                </td>
            </tr>
  ...
  <!-- /.content-wrapper -->
      ...
      <script>
          window.onload = function(){
              ...
              $(document).on('click','.galleryactivate',function (e) {
                  var button=this;
                  e.preventDefault();
                  $.ajax({
                      type:'post',
                      url:'{{route('admin.gallery.changestatus')}}',
                      data:{
                      ...
                      success:function (data) {
                          if(data.show==true) {
                              ...
                              $('#message').text(data.message);
                              if(data.action) {
                                  $(button).attr('class', 'btn-sm px-2 galleryactivate btn btn-' + data.btn);
