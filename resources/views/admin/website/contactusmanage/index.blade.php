...
@section('content')
  ...
  <div class="content-wrapper bg-white pt-1" style="min-height: 1416.81px;">
    ...
    <div class="card card-cascade narrower">
      ...
      <!--Card image-->
        <h2 class=" card card-header white-text text-center mx-3 bg-blue">Contact Us Page</h2>
          ...
          <!--Table-->
            ...
            <tr>
              <td>{{$details->address}}</td>
                ...
                <td id="formstatus" style="color: @if($details->form_status=='0') red @else green @endif">@if($details->form_status=='0')Not Active @else Active @endif</td>
              <td>
                  <div>
                  <a href="{{route('admin.contactusmanage.edit')}}" class="btn btn-outline-white btn-rounded btn-sm px-2" style="color:#007bff;">
                    <i class="fas fa-pencil-alt mt-0"></i>
                  </a>
                </div>
              </td>
                <td>
                    <a href="{{route('admin.contactusmanage.changestatus')}}" class=" formactivate btn btn-@if($details->form_status=='0')primary @elseif($details->form_status=='1')danger @endif  btn-sm px-2 m-0" style="color:white;">
                        @if($details->form_status=='0')Activate
                        @elseif($details->form_status=='1')Deactivate @endif
                    </a>
                </td>
            </tr>
             @endif
  ...
  <!-- /.content-wrapper -->
      ...
      <script>
          window.onload = function(){
              ...
              $(document).on('click','.formactivate',function (e) {
                  var button=this;
                  e.preventDefault();
                  $.ajax({
                      type:'post',
                      url:'{{route('admin.contactusmanage.changestatus')}}',
                      data:{
                          '_token':'{{csrf_token()}}'
                      },
                      success:function (data) {
                          if(data.show==true) {
                              ...
                              if(data.action) {
                                  $(button).attr('class', 'btn-sm px-2 formactivate btn btn-' + data.btn);
                                  $(button).text(data.action);
                                  $("#formstatus").text(data.status);
                                  $("#formstatus").css('color', data.statuscolor);
                              }
